...
public class AvgPassenger_Q4B {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (!columns[0].trim().isEmpty()) {
                ...
                try {
                    ...
                    if (dayOfWeek.toString() == "SATURDAY" || dayOfWeek.toString() == "SUNDAY") { // for weekend

                        // map hour_weekend_passengerkey to number of passengers to calculate per hour
                        context.write(new Text(hourVal + "_Weekend_" + passengerKey),
                                new DoubleWritable(Double.parseDouble(columns[3])));

                        // map hour_Weekend_tripKey to 1 to find total trips in the hour
                        context.write(new Text(hourVal + "_Weekend_" + tripKey), one);

                    }

                    else {
                        // map hour_weekday_passengerkey to number of passengers to calculate per hour
                        context.write(new Text(hourVal + "_Weekday_" + passengerKey),
                                new DoubleWritable(Double.parseDouble(columns[3])));

                        // map hour_Weekday_tripKey to 1 to find total trips in the hour
    ...
    public static class AveragePassengerPerHourReducer extends Reducer<Text, DoubleWritable, Text, DoubleWritable> {
        ...
        public void reduce(Text key, Iterable<DoubleWritable> values, Context context)
            ...
            if (key.find("Trip") > 0) {
                ...
                context.write(new Text(pkey[0] + "_" + pkey[1] + "_Passenger_Average"),
                        new DoubleWritable(totPassenger / sum));
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "average passenger per hour weekday weekend");
        job.setJarByClass(AvgPassenger_Q4B.class);
